...
const App = () => {
  ...
  const confidenceInterval = useMemo(() => {
    ...
    if (stats.type === 'twoPopulations') {
      ...
      const stdError = Math.sqrt((Math.pow(stats.pop1.stdDev, 2) / stats.pop1.sampleSize) + (Math.pow(stats.pop2.stdDev, 2) / stats.pop2.sampleSize));
      const z = 1.96; // 95% por defecto
